...
struct file *task_iter_lookup_next_fd_rcu(struct task_struct *task, unsigned int *ret_fd)
{
...
again:
	...
	for (;; curr_fd++) {
		struct file *f;
		f = task_iter_lookup_next_fd_rcu(curr_task, &curr_fd);
		if (!f)
			break;
		if (!get_file_rcu(f))
			continue;
...
task_vma_seq_get_next(struct bpf_iter_seq_task_vma_info *info)
{
	...
	if (info->task) {
		 ...
		 *         VMA2'.
		 */
if (rwsem_is_contended(&curr_task->mm->mmap_sem)) {
			info->prev_vm_start = curr_vma->vm_start;
			info->prev_vm_end = curr_vma->vm_end;
			op = task_vma_iter_find_vma;
up_read(&curr_task->mm->mmap_sem);
if (down_read_killable(&curr_task->mm->mmap_sem))
				goto finish;
		} else {
